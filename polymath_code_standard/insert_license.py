...
FUZZY_MATCH_TODO_COMMENT = ' TODO: This license is not consistent with the license used in the project.'
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        '--use-current-year',
        action='store_true',
        help=('Use the current year in inserted and updated licenses, implies --allow-past-years'),
    )
    parser.add_argument(
        '--allow-past-years',
        action='store_true',
        help=('Allow past years in headers. License comments are not updated if they contain past years.'),
    )
    ...
    if check_failed:
        ...
        if todo_files:
            print(f'Some sources contain TODO about inconsistent licenses: {todo_files}')
...
def get_license_info_list(args) -> list[LicenseInfo]:
    ...
    for filepath in args.license_filepath:
        ...
        prefixed_license = [f'{comment_prefix}{extra_space if line.strip() else ""}{line}' for line in plain_license]
...
def license_not_found(
    ...
    if not remove_header:
        ...
        for index, line in enumerate(src_file_content):
            ...
            index = 0
        src_file_content = (
            src_file_content[:index] + license_info.prefixed_license + [license_info.eol] + src_file_content[index:]
        )
...
def try_update_year(line: str, filepath: str, current_year: int, introduce_range: bool) -> str | None:
    ...
    if matches:
        ...
        if end_year and int(end_year) < current_year:
...
def license_found(
    ...
    if remove_header:
        ...
        if last_license_line_index < len(src_file_content) and src_file_content[last_license_line_index].strip():
            ...
            src_file_content = (
                src_file_content[:license_header_index]
                + src_file_content[license_header_index + len(license_info.prefixed_license) + 1 :]
            )
        updated = True
...
def fail_license_todo_found(src_file_content, fuzzy_match_todo_comment, top_lines_count):
    for i in range(top_lines_count):
...
def fuzzy_find_license_header_index(
    ...
    for i in range(top_lines_count):
        ...
        license_string_candidate, candidate_offset = get_license_candidate_string(candidate_array, license_info)
        ...
        if not in_license:
            if stripped_comment_start:
                if stripped_line.startswith(stripped_comment_start):
                    in_license = True
                    found_license_offset = current_offset + 1
                    continue
            else:
